import SamplePlayer, { SamplePlayerEvent } from 'audio/SamplePlayer';
...
export default class Scheduler extends EventDispatcher {
	...
	public start(): void {
		...
		this.scheduleIntervalId = window.setInterval(this.schedule.bind(this), this.scheduleInterval * 1000);
		this.schedule();
	...
	public getScheduleDataListForRing(
		...
		while (ringItemIndexIterator < 100) {
			...
			const itemRevolution = currentRevolution + Math.floor(ringItemIndexIterator / ring.items.length);
...
export const setupSchedulerStoreCommunication = (scheduler: Scheduler, store) => {
	...
	scheduler.samplePlayer.addEventListener(SamplePlayerEvent.UPDATE_LAST_SCHEDULED_REVOLUTION, event => {
