...
import { IDisc, IInteractable, InteractableType, IPoint, IRingItem, ISizeData, IStore } from '../../data/interface';
...
export default class DiscInteractionManager extends EventDispatcher {
	...
	private updateHighlight(): void {
		...
		const distanceToCenterFactor = getDistanceToCenterFactorForPoint(this.mousePosition, this.sizeData);
		...
		if (!this.store.state.interaction.selection) {
			...
			if (this.disc.sound.sample) {
				const slice = getSliceForPosition(this.mousePosition, this.sizeData, this.disc.sound.slices);
		...
		if (highlight && highlight.type === InteractableType.RING_ITEM) {
			...
			new ValueDragger(
				...
				volume => {
					...
					this.store.commit(discStore.mutations.setRingItemVolume, {
						volume,
						ringItem: highlight,
					});
	...
	onMouseUp = () => {
		...
		switch (highlight.type) {
			...
			case InteractableType.SLICE: {
				if (selection) {
					if (selection.type === InteractableType.DISC) {
						...
						this.store.commit(discStore.mutations.toggleSliceInRing, {
							ring: selection,
							slice: highlight,
						});
					}
			...
			default: {
				console.error('Unknown highlight type', highlight);
			}
		}
