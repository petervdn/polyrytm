import { ISizeData, IDisc, IStore, IInteractable, InteractableType, IRing, IRingItem } from '../../data/interface';
...
export default class RingsRenderer extends AbstractRenderer {
	...
	private updateRotation(): void {
		if (this.cssRotate) {
			this.canvas.style.transform = `rotate(-${this.scheduler.timeData.currentRevolutionDegrees}deg)`;
		} else {
			this.draw();
		}
	}
	...
	public draw(): void {
		...
		if (this.store.state.scheduler.isPlaying) {
			this.disc.rings.forEach(ring => {
				...
				const progressInItemFactor = (playRadians - itemStartRadians) / (itemEndRadians - itemStartRadians);
				const opacity = (1 - progressInItemFactor) ** 2;

				if (opacity > 0.1) {
					drawRingItem(this.context, ring.items[activeItemIndex], this.sizeData, `rgba(0,0,0,${opacity}`);
				}
			});
		}
		...
		drawPlayhead(this.context, this.sizeData, this.disc, this.store.state.setting.playMode, this.scheduler);
	}
		...
		if (selectedRing && selectedRing.disc === this.disc) {
			...
			this.disc.rings.forEach((ring, index) => {
				if (ring !== selectedRing) {
					drawRing(this.ringsCanvasContext, ring, index, this.sizeData, this.store.state.theme.activeTheme);
				}
			});
