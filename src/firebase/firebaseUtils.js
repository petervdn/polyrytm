...
export const initUserLogin = store =>
	new Promise(resolve => {
		firebase.auth().onAuthStateChanged(authUser => {
			if (authUser) {
				const firebaseUserRef = firebase.database().ref(`/${firebasePath.database.USERS}/${authUser.uid}`);
...
export const storeRytm = (rytm, isPublic, user) =>
	new Promise((resolve, reject) => {
		...
		const rytmsPath = isPublic ? firebasePath.database.PUBLIC_RYTMS : firebasePath.database.PRIVATE_RYTMS;
		...
		if (user) {
			...
			const shortRytmInUserPath = `${firebasePath.database
				.USERS}/${authUser.uid}/${rytmsShortPath}/${newRytmKey}`;
