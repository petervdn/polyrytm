...
import { DiscSizeData, Point } from '../data/interfaces';
 ...
 * @returns {number}
 */
export function getRingIndexForPosition(position: Point, sizeData: DiscSizeData): number {
  const distanceToCenter = distance(position.x, position.y, sizeData.halfSize, sizeData.halfSize);
  const distanceToOuterRing = sizeData.ringsOuterRadius - distanceToCenter;

  return Math.floor(distanceToOuterRing / sizeData.ringSize);
}
//
// /**
 ...
 * @returns {IPoint}
 */
export function getMousePositionByMouseEvent(event: MouseEvent): Point {
